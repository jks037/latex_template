...
%%%%%%%%% TITLE
%%%%%%%%% TITLE
\title{A weak-shot classification practice based on modified sample weighting}
...
Just follow the method Yen-Chang Hsu and Kira proposed in 2018 ~\cite{HsuLK18}, we choose an end-to-end deep network SimNet to model the similarity function. Firstly, SimNet takes in a mini-batch of images and outputs the pairwise similarity/dissimilarity score for each pair of input images,if mini-batch size is $A$,then there are $A\times A$ pairs for the mini-batch size A. Secondly,the enumeration layer simply concatenates each pair of image features which are extracted by the backbone. For instance, suppose the image features have size $A\times B$ with the feature dimension B, then the output of the enumeration layer will have size $A\times A\times 2B$. Thirdly, one fullyconnected (FC) layer extracts feature for each concatenated feature pair from enumeration layer, which is represented as relation feature of each pair. Finally, the relation features are supervised by a binary relation classification loss with binary labels being “similar” ,which means a pair of images from the same category and “dissimilar” which means a pair of images from different categories. Considering the training and testing for SimNet, if we construct the mini-batch randomly, imbalanced problem about similar and dissimilar pairs should be considered. Specifically, if there are $C$ categories in a mini-batch, the probability for two images to be from the same category is $1/C$ . To reduce the imbalance between similar pairs and dissimilar pairs, when constructing a mini-batch, we first randomly select $D$ (less than $C$) categories and then randomly select $C/D$ images from each selected category, as in (Yen-Chang Hsu and Kira 2018). We use $D = 10$ and $C = 100$ for both training and testing.

\subsubsection{Adversarial Loss}
Ideally, the learnt similarity is category-agnostic. However, there may be domain gap between the relationship features of different category sets. Therefore, we use the novel training set to further narrow the domain gap between the base category and the novel category. Therefore, the discriminator is used to classify the relationship features, but it does not consider where these relationship features come from, whether base categories or novel categories. The SimNet acts as generator, aiming to produce relation features which could not only confuse the discriminator but also benefit the relation classification. We exclude the image pairs from novel categories from the relation classification loss to reduce the noise of the labels of novel training images. We optimize SimNet and the discriminator alternatingly in an adversarial manner. In order to minimize the relation classification loss of similarity net and maximize the adversarial loss of discriminator,we freeze the generator and minimize the adversarial loss of discriminator. The classification loss and the adversarial loss are balanced with a hyper-parameter β, set as 0.1 via cross-validation. 


\subsection{Learning Classifier on Noisy Training Set}
\label{section:method:class}
Due to the errors in training labels which is common for web images, the performance is far from satisfaction when train directly. According to prior study~\cite{sukhbaatar2015training}, noise can be devided into two types: outlier noise and label-flip noise. In our experiments, training sets consists of images collected by search engines, thus many of them are qualified. We take advantage of pre-trained similarity net to deal with these outliers, as similarity among them is a powerful property that help filter the original noisy set and get a relatively clear one. We try different weighting methods based on similarity matrix rather than applying a plain averaging. We will then introduce the detailes.
...
%\end{itemize}
\subsubsection{Adjusted Sample Weighting}
\label{section:method:class:msw}
...
For the \emph{c}-th novel category, we first compute the sililarity matrix $S_c$, whose size is $N_c^n\times N_c^n$, $N_c^n$ being the number of images belongs to \emph{c}-th novel category. This is computationally efficient as it just need to be computed once for each category. Then we compute teh average of similarities between image \emph{i} and each of the rest images:\begin{equation}w_{c,i} = \cfrac{1}{N_c^n}\sum_j^{N_c^n}\cfrac{s_{c,i,j}+s_{c,j,i}}{2}.   \label{eql:simw}\end{equation} Then we modify this average in different manners to enlarge the weights for clear images and to reduce weights for noisy ones.

\paragraph{\textbf{Truncated manner.}} This manner assumes that when similarity average is beyond some threshold, the corresponding images are well-labeled images and treats them equally. Similarly images with average below some threshold are considered having negative impact on training and are excluded by setting their weights to zeros. For other images, we normalized them to fit the range from zero to upper threshold. Denote the trncated weight by $tw$, it can be represented as:
\begin{equation} tw_{c,i}=\begin{cases}
    w_{c,u},& \text{$w_{c,i} \geq w_{c,u}$}\\
    w_{c,i}/w_{c,u},& \text{$w_{c,l}<w_{c,i}\leq w_{c,u}$}\\
    0,& \text{$w_{c,i}\leq w_{c,l}$}
\end{cases}.
\label{eq:tr}
\end{equation}
Here $w_{c,u}$ denotes a chosen upper threshold that, if $w_{c,i}$ overcome this, we regard the correspongding image is well-labeled, and all images satisfying this is treated equally. Similary, $w_{c,l}$ denotes a chosen lower threshold that, images with $w_{c,i}$ lower than $w_{c,l}$ are regarded as outliers to remove away from learning process. Images with intermediat values are normalized using the upper threshold. 

In experiment, to choose $w_{c,u}$ and $w_{c,l}$, which represent threshold for clear images and totally noisy images respective, we perform an emperical study about the training set. First, we randomly choose 10 categories in novel sets; Then, we randomly select 100 images and manully label them to clear, noisy and intermediat; Finally, we compute the propotion of the clear images and noisy images to determine the upper and lower bound. Before labeling, we manully look up the base dataset of selected categories for precise understands. By the way, the labeled subset provides reference for evaluating the effect of weighting.

\paragraph{\textbf{Quadratic manner.}} In this manner we apply quadratic function on averages. As average are non-negative, this will emphasize the larger weights and alleviate the others. Denote by $qw_{c,i}$, the weight can be computed as:
\begin{equation} 
    qw_{c,i}=w^2_{c,i}/C_c
\label{eq:qua}
\end{equation}
Here $C_c$ is a constent normalizing the weight to a plausible range, which differs in different category.

\paragraph{\textbf{Trigonometric manner.}} In this manner, we use sine function from phase $-\frac{\pi}{2}$ to $\frac{\pi}{2}$. Values of averages are rerange so that we can apply this. Different from quadratic function, it has a relatively flat section at both end of its domain. Therefore, it tends to cancel the difference among weights when they are close to maximum or minimum. In this means, trigonometric function acts similar to equation (2), and is free from manually select thresholds. Denote by $sw_{c,i}$, the modified weight can be work out by:
\begin{equation} 
    sw_{c,i}=C_c \times [sin(\frac{w_{c,i}-max(w_c)/2}{max(w_c)} \times \pi)+1]
\label{eq:sin}
\end{equation}
Note here we assume the weights before modification is non-negative. The weights are mapped to $[-\frac{\pi}{2},\frac{\pi}{2}]$ to ensure monotonically increasement. $C_c$ is a constent for normalization.

Note that these three manners use monotonically increasing functions to modify the weights computed by averages elements of similarity matrix. In experiments, we also use the original averages for contrast, represented by $w_c$. The original sample weights $w_c$ are normalized to have unit mean. The other modified weights are normalized to have identical ranges, \emph{i.e.}, maximums of $tw_c$, $qw_c$, $sw_c$ are identical and equal to $max(w_c)$ after normalization. We normalize the way for convenience of adjusting hyperparameters. Figure \ref{method:example} shows effects of modifications performed on an linear weight.  

\begin{figure}[t]
\includegraphics[width=0.47\textwidth]{figures/weight example.png}
\caption{An example of weights after modification. We assume weights varify from 0 to 1 uniformly here for demonstration.}
\label{method:example}
\end{figure}

\subsubsection{Selective Sample Weighting}
In the previous section, an image is weighted high if many other images in its category are similar to it. For high-quality images, this strategy works well because images similar to them must exist and then contribute large for their weights. However, for images of lower quality, simply averaging similarities may fail because it is likely that a smaller group of similar outliers exist. Similar fail happens an image is similar to a large number of other images in a moderate level. These results in inaccurate weighting for images of moderate quality.

We apply a two-step weight strategy to alleviate these negative impacts. (1) Select $n$ images which have the highest weights as well-labeled images; (2) Perform weighting in section \ref{section:method:class:msw} without modification neglecting similarities with non-selected images. Denote this type of weight by $w'_{c}$, we can compute it as:
\begin{equation}
    w'_{c,i}=\cfrac{1}{n}\sum_j'^{n}\cfrac{s_{c,i,j'}+s_{c,j',i}}{2}.
\label{eq:se}
\end{equation}


Here $j'$ represents indexes of the selected images in the first step. The value choise of $n$ is performed in a similar process of choosing thresholds for truncated weighting, \emph{i.e.} by manually label a sampled subset of the dataset. 

\subsection{The Full Objective}
\label{section:method:full}
On the whole, the classifier is trained on novel set by cross-entropy loss combined with truncated sample weighting:
\begin{equation}\mathcal{L}_q = 
    \cfrac{1}{B}\sum_{i=1}^{B}-\overline{w}_{c,i}logf_{c,i}(x)_c\label{eql:gloss}, \end{equation} 
where $\overline{w}_c$ is $tw_c,qw_c,qsw_c$ or $w'_c$ corresponding to equation (2), (3), (4) and (5).
